...
/* This program demonstrates the use of mutex locks and conditional variables. It assumes that LEDs are connected to pins 6 and 9.
  ...
  As Arduino routines are not reentrant, we use a binary semaphore to enforce atomic access to the printSerial routine.
...
void printSerial(unsigned int taskNum, unsigned int count)
{
  ...
  OSTakeSema(&sema);
  sprintf(buffer, "Task %u: %u times.", taskNum, count);
...
void setup()
{
	...
	OSCreateConditional(&c1);
  	OSCreateConditional(&c2);
        ...
        // Create the semaphore that controls access to
        // serial printing. Create a binary sema with initial value of 1
        OSCreateSema(&sema, 1, 1);
